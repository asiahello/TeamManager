...
$(".editable").on('dblclick', 'li', function () {
    oriVal = $(this).text();
    $(this).text("");
    $("<input type='text'>").appendTo(this).focus();
});

$(".editable").on('focusout', 'li > input', function () {
    var $this = $(this);
...
var add_opportunities = document.getElementById('add_opportunities');
var add_threats = document.getElementById('add_threats');

add_opportunities.addEventListener('click', function() {
    var itemsByTagName = document.getElementsByTagName("li");
    opportunities.innerHTML += '<li class="list-group-item"></li>'
});

add_threats.addEventListener('click', function() {
    var itemsByTagName = document.getElementsByTagName("li");
    threats.innerHTML += '<li class="list-group-item"></li>'
});
...
(function() {
    ...
    // a reference to the selected items' owning container and a refernce to the current drop target container
    var selections = {
        items      : [],
        owner      : null,
        droptarget : null
    ...
    function addSelection(item) {
        ...
        if(!selections.owner) {                                 //if the owner reference is still null, set it to this item's parent
            selections.owner = item.parentNode;                 //so that further selection is only allowed within the same container
        }
        else if(selections.owner != item.parentNode) {          //or if that's already happened then compare it with this item's parent
            return;                                             //and if they're not the same container, return to prevent selection
        }

        item.setAttribute('aria-grabbed', 'true');              //set this item's grabbed state
        selections.items.push(item);                            //add it to the items array
    }
    ...
    function removeSelection(item)
    {
        ...
        for(var len = selections.items.length, i = 0; i < len; i ++) {      //then find and remove this item from the existing items array
    ...
    function clearSelections()
    {
        if(selections.items.length) {                                       //if we have any selected items
            ...
            for(var len = selections.items.length, i = 0; i < len; i ++) {  //reset the grabbed state on every selected item
                selections.items[i].setAttribute('aria-grabbed', 'false');
            }

            selections.items = [];                                          //then reset the items array
        }
    }
